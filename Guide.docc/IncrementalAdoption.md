...
私たちは途中で出てくる問題に対処しながら並行処理機能を _徐々に_ 導入できます。
...
非同期コンテキストから直接使用可能であるようなバージョンの関数を作れます。
...
> 一方、チェックされた継続を2回以上再開すると、意図的にクラッシュが発生して未定義の動作を防ぎます。
...
`withCheckedContinuation` 関数は、非同期ではないコードと非同期のコードをつなぎ合わせられるようにするために存在する[一連の標準ライブラリAPI][continuation-apis]の1つです。
...
動的隔離は、データの隔離を表現するためのフォールバックとして使用できる、実行時のメカニズムを提供します。
...
しかし、まだ移行していない他の場所でこの型が使われている場合、ここに静的隔離を追加すると追加の変更が大量に必要になる可能性があります。
...
静的隔離と動的隔離を組み合わせることで、変更範囲を緩やかにするための強力なツールとなります。

### 明示的なMainActorコンテキスト

`assumeIsolated` メソッドは同期型で、想定が間違っていた場合に実行を阻止することで隔離情報を実行時から型システムに回復させるために存在します。 `MainActor` 型には、非同期のコンテキストにおいて隔離を手動で切り替えるために使用できるメソッドもあります。
...
静的隔離のおかげで、必要に応じて隔離を切り替えるというプロセスをコンパイラが検証し自動化できることを忘れないでください。
たとえ静的隔離と組み合わせて使う場合でも、本当に `MainActor.run` が必要であるときを見極めるのは難しいかもしれません。
...
クロージャの送信可能性はコンパイラがクロージャ本体の隔離をどのように推論するかに影響を与えます。
...
このコードは問題なくコンパイルできますが実行時にクラッシュします。
...
アクターの隔離が変更されるかもしれないとコンパイラが知ったため、呼び出し元でタスクを用意しタスク内で待機する必要があります。
...
これは実行時の動的アクター隔離の強制をすべて抑制します。

> 警告: このフラグの使用には注意が必要です。
これらの実行時確認を無効にするとデータ隔離違反を許すことになります。
...
このように `@preconcurrency` を使用すると、完全な確認が有効になっているクライアントモジュール上では隔離のマーキングが働きます。
...
Foundationをインポート可能なプロジェクトと連携しているなら、 `NSObjCRuntime.h` にある次のアノテーションマクロが使用できます：
...
SDKやその他のObjective-CライブラリはSwiftの並行処理を導入することで進歩している一方、ドキュメントでしか説明されていなかった取り決めをコードで表現するようになることがあります。
例えば、Swiftに並行処理が導入される以前は、「これは常にメインスレッド上で呼び出される」というようなコメントでスレッドの動作をドキュメント化しなければならないことがよくありました。

Swiftの並行処理により、これらのコードコメントはコンパイラと実行時で強制される隔離確認に変換でき、そのようなAPIを導入するときにSwiftが検証するようになります。
...
しかし、この例において、過去に次のようにドキュメント化されたメソッドを考えてみましょう：
...
メインアクターで呼び出される場合と呼び出されない場合があるという、異なるスレッド戦略が明確にドキュメント化されていますが、メソッドへこれらのセマンティクスをアノテーションするのをうっかり忘れてしまいました。
...
final class MyJetPack: NSJetPack {
  override class var supportsHighAltitude: Bool { // Swift6では実行時にクラッシュ
    true
  }
}
```

上記のコードは実行時確認でクラッシュします。この確認は、Objective-C上のSwiftの並行処理ではない領域からSwiftに移動するときに、メインアクターで実際に実行されていることを確認するために行なわれるものです。

これは、自動的にそのような問題を検出し見込みが破られた際に実行時にクラッシュするというSwift 6の機能です。
そのような問題を未診断のままにしておくと、現実の検出困難なデータ競合を実際に引き起こす恐れがあり、データ競合安全に関するSwift 6の保証を損ないます。
...
ライブラリがアノテーションの問題を修正するまでは、正しく `nonisolated` としたメソッドを用いて、次のようにメソッドを証言できます：
...
Dispatchや他の並行処理ライブラリで使い慣れているだろう一部のパターンは、Swiftの構造化同時並行処理モデルの世界に適合させるために、形を変える必要があるかもしれません。
