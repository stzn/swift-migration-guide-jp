...
静的隔離のおかげで必要に応じて隔離を切り替えるというプロセスをコンパイラが検証し自動化できることを忘れないでください。
...
コンパイラはアクターの隔離が変更されるかもしれないことを知っているため、呼び出し元でタスクを用意しタスク内で待機する必要があります。
...
このように `@preconcurrency` を使用すると、完全な並行性の確認が有効になっているクライアントモジュール上では隔離のマーキングが働きます。
